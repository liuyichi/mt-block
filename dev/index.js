...
let elementName = location.search.slice(1) || 'bill';
switch (elementName) {
  case "text":
  case "password":
  case "number":
...
let context = require.context('../', true, /^\.\/((?!dev\/).+\/demo(?:.js|\/index.js)|log.js)$/g);
let module = context(context.keys().find(m => m.includes(`/${elementName}/demo`) || m.includes(`/common/${elementName}/demo`) || m.includes(`/${elementName}.js`)));
