...
export function loopAllChildren(childs, callback, parent) {
  const loop = (children, level, _parent) => {
    ...
    React.Children.forEach(children, (item, index) => {
      ...
      callback(item, index, pos, item.props.id || item.key || pos, getSiblingPosition(index, len, {}), _parent);
    });
  };
...
export function handleCheckState(obj, checkedPositionArr, checkIt) {
  ...
  let objIds = Object.keys(obj);
  // let s = Date.now();
  objIds.forEach((i, index) => {
  ...
  // console.log(Date.now()-s, checkedPositionArr.length * objIds.length);
  objIds = objIds.filter(i => i); // filter non null;
  ...
  // console.log(Date.now()-s, objIds.length, checkIt);
}
...
export function getCheck(treeNodesStates) {
  ...
  Object.keys(treeNodesStates).forEach((item) => {
    ...
    if (itemObj.checked) {
      checkedIds.push(itemObj.key);
      checkedNodes.push(itemObj.node);
      checkedNodesPositions.push({ node: itemObj.node, pos: item });
    } else if (itemObj.halfChecked) {
      halfCheckedIds.push(itemObj.key);
    }
  });
  return {
    halfCheckedIds, checkedIds, checkedNodes, checkedNodesPositions, treeNodesStates,
  };
}

export function getStrictlyValue(checkedIds, halfChecked) {
  if (halfChecked) {
    return { checked: checkedIds, halfChecked };
  }
  return checkedIds;
}
