...
(function (factory) {
	...
	var SortableMixin = {
		...
		componentDidMount: function () {
				...
				copyOptions[name] = function (evt) {
					if (name === 'onStart') {
						...
						if (name === 'onAdd') {
							...
							if (_activeComponent._sortableInstance.lastPullMode != 'clone') {
								remoteState[_getModelName(_activeComponent)] = remoteItems;
							} else {
								//FIXME clone的时候造成react操作removeChild报错
								 evt.from.contains(evt.item) && (evt.from.removeChild(evt.item));
							}
						}
						else {
			...
			DOMNode = 'getDOMNode' in this ? ReactDOM.findDOMNode(this.refs[options.ref] || this) : this.refs[options.ref] || this;
